...
- 收敛第一版产品闭环：游戏内 Lua 直接运行本地知识库评分器，不要求玩家额外启动 Python 服务；支持“小马流、学院流、大商路流”评分、排序、阻断原因、下一步检查项和游戏内可查看数据位置。
- 补充游戏内标准化字段：`horseCount`、`ironCount`、`averageCityProduction`，用于减少流派判断对展示文案的依赖。
- 修复提问框连续输入问题：提交一次问题后会重置输入框，但保留刚提交的问题和回答，用户已确认可以继续输入后续问题。
- 本地版本策略：GitHub 推送暂时受网络影响，先用本地 commit/tag 保存可回退版本点；版本台账见 `docs/LOCAL_VERSION_CHECKPOINTS.md`。
- 扩展本地知识库与游戏内兜底评分器：从 3 个 MVP 流派扩展到 8 个流派，包括小马流、学院流、大商路流、征服流、宗教流、文化流、工业流、扩张流。
- 新增 Python Consul 回答编排层：可输出玩家可读回答、原始 matcher 结果、后续大模型可用的 `ai-context`，并明确回答契约：解释依据、不替玩家操作、包含游戏内可查看数据位置、暴露不确定性。
- 新增 Python Consul 本地 HTTP 服务边界：`obelisk_knowledge.local_service` 提供 `/health` 和 `/answer`，后续可作为 Overlay 或伴随进程调用入口；游戏内 Lua 仍保持独立可用。
...
1. 实机验收 8 流派本地回答：征服、宗教、文化、工业、扩张问题是否能连续提问并返回可解释评分。
2. 后续开始逐批采集网络玩家攻略，并先进入 `knowledge/review/` 做规则校验。
3. 后续按真实提问暴露的数据缺口继续补专项字段。
4. GitHub 网络恢复后统一推送本地 commit 和 tag。
