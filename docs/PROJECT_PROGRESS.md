...
1. 完成 AI 回答链路设计：当前快照数据 + 底层规则 + 玩家攻略检索 + 可解释建议。
2. 设计本地运行边界：游戏内 Lua 继续保持可独立运行；Python Consul / AI 服务作为可选增强，不影响基础提问条可用性。
3. 补齐知识库检索到回答层的中间结构：匹配结果、证据、置信度、游戏内可查看位置。
4. 后续开始逐批采集网络玩家攻略，并先进入 `knowledge/review/` 做规则校验。
5. 后续按真实提问暴露的数据缺口继续补专项字段。
6. GitHub 网络恢复后统一推送本地 commit 和 tag。
